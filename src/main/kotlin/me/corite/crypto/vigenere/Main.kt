package me.corite.crypto.vigenere
...
class Main : Action {
    ...
    override fun main() {
        val text1 = readFile(filePrefix + "Lorem1.txt")
        val text2 = readFile(filePrefix + "Lorem2.txt")
        val text3 = readFile(filePrefix + "Lorem3.txt")
    ...
    fun getCoincidenceIndex(text: String): Double {
        ...
        return 1.0 / (n * (n - 1)) * distinctValues.sumOf {
            getAbsoluteFrequency(text, it) * (getAbsoluteFrequency(
                text,
                it
            ) - 1)
        }
    ...
    fun getKeyLength(text: String): Int {
        ...
        for (keyLength in 1..100) {
            ...
            for (remainder in 0 until keyLength) {
                ...
                val textSnippet = text.chars().toArray().filterIndexed { index, _ -> index % keyLength == remainder }
    ...
    fun findCorrectKeyLength(lengthCoincidenceMap: Map<Int, Double>): Int {
        val lengthsToConsider =
            lengthCoincidenceMap.entries.filter { (it.value - 0.06).absoluteValue < 0.015 }.map { it.key }.toIntArray()
    ...
    fun getEncryptedTextsWithSameKey(text: String, keyLength: Int): Array<String> {
        ...
        for (i in 0 until keyLength) {
            textsWithSameKey[i] =
                text.chars().toArray().filterIndexed { index, _ -> index % keyLength == i }.map { it.toChar() }
                    .joinToString("")
